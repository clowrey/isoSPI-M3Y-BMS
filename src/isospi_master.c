 ...
 * Adapted from C++ to C for RP2350, using PIO1 SM0.
 */
...
// Use PIO1 SM0 (PIO0 is used by CAN interface)
...
void isospi_master_setup(uint tx_pin_base, uint rx_pin_base) {
    ...
    printf("isoSPI Master: Initialized on PIO1 SM0 (TX: GP%d-GP%d, RX: GP%d-GP%d)\n", 
