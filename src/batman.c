 ...
 */
// CL: Helper functions moved to isospi_master.c
// Wrapper to use isospi_get_data with the global response_buffer
static void isospi_get_data_wrapper(uint8_t reg_cmd) {
    // Initialize buffer to 0x00
    memset(response_buffer, 0x00, 72);
    
    // Call the library function with response_buffer
    isospi_get_data(reg_cmd, (char*)response_buffer, 12);
}
...
static void batman_get_data(uint8_t reg_cmd) {
    ...
    // Read 12 bytes of response (6 words for 3 cells)
    // Keep CS asserted and read continuously
    for (int count = 0; count < 12; count += 2) {
        // Send 16-bit padding (0x0000) to clock out response
        tx_buf[0] = 0x00;
        tx_buf[1] = 0x00;
        spi_write_read_blocking(TESLA_BMS_SPI_INST, tx_buf, rx_buf, 2);
...
static void isospi_simple_test(void) {
    ...
    // Step 4: Read voltage register A
    isospi_get_data_wrapper(CMD_READ_A);
